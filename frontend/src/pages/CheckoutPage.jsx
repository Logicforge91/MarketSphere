...
export default function CheckoutPage() {
  ...
  const regionalDeliveryMethods = deliveryMethods.filter((item) => region.shipping.includes(item.id));
  const regionalPaymentMethods = paymentMethods.filter((item) => region.payments.includes(item.id));
  ...
  const calculations = useMemo(() => {
    ...
    const tax = Math.round(taxable * region.taxRate);
  ...
  }, [cart, cartTotal, coupon, giftCard, giftCards, giftWrap, membership, method.id, method.price, orders, paymentMethod, region, rewardPoints]);
      ...
      <div className="advanced-checkout-layout">
        <section className="checkout-stage">
          ...
          {step === 3 && <div className="checkout-section"><div className="checkout-section-title"><CreditCard /><div><span>Step 3</span><h1>Payment and savings</h1><p>Apply benefits and choose how you want to pay.</p></div></div>
            ...
            <h2>Payment method</h2><div className="checkout-payment-methods">{regionalPaymentMethods.map((item) => { const MethodIcon = item.icon; return <label className={paymentMethod === item.id ? "selected" : ""} key={item.id}><input type="radio" checked={paymentMethod === item.id} onChange={() => { setPaymentMethod(item.id); setPaymentError(""); setPaymentStatus("idle"); }} /><MethodIcon /><div><strong>{item.name}</strong><span>{item.detail}</span></div>{item.id === "saved-card" && <b>Default</b>}</label>; })}</div>
